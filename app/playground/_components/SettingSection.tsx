...
const SettingSection = ({selectedEl,clearSelection}:Props) => {
...
const addClass = () => {
  ...
  return (
    <div className=' w-96 shadow p-4'>
      <h2 className='flex items-center font-bold'>
        <SwatchBook/>SettingSection
      </h2>
      <div className='flex items-center gap-4'>
          <div className='flex-1'>
               <label>Font size</label>
      <Select defaultValue={selectedEl?.style?.fontSize||'24px' } onValueChange={(value)=>{applyStyle('fontSize',value)}}>
  ...
  <SelectContent>
    <SelectGroup>
      ...
      <input onChange={(event)=>{applyStyle('color', event.target.value)}} type='color' className='w-7 h-7 rounded-2xl' />
      </div>
